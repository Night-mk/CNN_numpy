...
class BatchNorm(object):
    def __init__(self, input_shape, learning_rate=0.0001):
        ...
        # 需要将输入size转换为[batch, -1]
        # self.input_shape_col = input_shape_col
        # self.bn_shape = self.input_shape_col[-1] 

        self.input_data = np.zeros(self.input_shape)

        # 初始化BN层需要的参数gamma、beta,参数的数量和均值、方差后的维度相同
        self.gamma = np.ones(self.channel_num)
        self.beta = np.zeros(self.channel_num)

        self.gamma_grad = np.zeros(self.channel_num)
        self.beta_grad = np.zeros(self.channel_num)

        self.epsilon = 1e-5
        self.mean = np.zeros(self.channel_num)
        self.var = np.zeros(self.channel_num)

        # 初始化指数加权移动平均的mean和var，用于在测试环节计算样本均值和方差的无偏估计
        self.moving_mean = np.zeros(self.channel_num)
        self.moving_var = np.zeros(self.channel_num)
    ...
    def forward(self, input_array, mode="train"):
        ...
        for i in range(self.channel_num):
            output_y_i = self.gamma[i]*self.normed_x[:,i,:,:] + self.beta[i]
    ...
    def gradient(self, eta):
        ...
        for i in range(self.channel_num):
            normed_x_grad_i = self.gamma[i]*self.eta[:,i,:,:]
...
def bn_test():
    ...
    bn1 = BatchNorm(a.shape, learning_rate=0.0001)
